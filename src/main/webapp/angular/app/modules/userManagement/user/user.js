...
angular.module('user', [
    ...
    .controller('user', ['$rootScope', '$scope', '$stateParams', 'Data', function ($rootScope, $scope, $stateParams, Data) {
        'use strict';
        ...
        if ($stateParams.id) {
            ...
            $scope.data.get({section: 'users', id: $rootScope.id}, function (data) {
                $scope.user = data;
            });
        }
        ...
        $scope.save = function () {
            ...
            $scope.data.post({section: 'users', id: $rootScope.id}, function (data) {
    ...
    .directive('player', function () {
        ...
        return {
            ...
            template: '<video preload="none" poster="{{ videos[0].poster }}">' +
                            '<source ng-repeat="user in videos" ng-src="{{ user.src | trusted }}" type="video/{{ user.type }}" />' +
